...
if [[ $EUID -ne 0 ]]; then echo "Run with sudo: sudo ./build.sh" >&2; exit 1; fi
. /etc/os-release
if [[ "${ID:-}" != "debian" || "${VERSION_CODENAME:-}" != "trixie" ]]; then
  echo "Warning: B2 is validated on Debian 13 (trixie). Continuing anyway." >&2
fi
...
if ((${#missing[@]})); then printf 'Missing Debian packages: %s\n' "${missing[*]}" >&2; exit 1; fi
...
rm -rf "$BUILD"; mkdir -p "$BUILD" "$OUT"; cp -a "$ROOT/config" "$BUILD/config"; cd "$BUILD"
...
[[ -n "$ISO" ]] || { echo "Build completed but no ISO was found." >&2; exit 1; }
